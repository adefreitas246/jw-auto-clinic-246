...
function CatalogTile({
  ...
  return (
    ...
    >
      ...
      <Text style={[k.tileName, selected && k.tileNameSelected]} numberOfLines={2}>
        {item.name}
      </Text>
      {!!item.description && (
        <Text style={k.tileDesc} numberOfLines={1}>{item.description}</Text>
      )}
...
export default function WalkinKioskScreen() {
    ...
    (async () => {
      try {
        ...
        const pkgItems: CatalogItem[] = pkgRes.data.map(p => ({
          ...
          durationMinutes: p.serviceIds.reduce((s, sv) => s + sv.duration, 0) || 30,
        }));
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setCountdown(c => {
        if (c <= 1) { clearInterval(id); resetToIdle(); return CONFIRM_RESET_SEC; }
        return c - 1;
      });
    }, 1000);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Oops!', err.response?.data?.error ?? 'Could not add to queue. Please try again.');
    } finally {
      setSubmitting(false);
    }
        ...
        <Pressable style={{ flex: 1 }} onPress={() => goTo(1)}>
          ...
          >
            <SafeAreaView style={k.idleInner}>
              ...
              {/* Main icon */}
              <Animated.View style={[k.idleIconWrap, { transform: [{ scale: pulseAnim }] }]}>
                <Ionicons name="car-sport-outline" size={72} color="#fff" />
              </Animated.View>

              <Text style={k.idleTitle}>Walk-in Service</Text>
              <Text style={k.idleSub}>Join the queue and we'll get you sorted</Text>

              {catalogBusy
                ? <ActivityIndicator color="rgba(255,255,255,0.7)" style={{ marginTop: 40 }} />
                : (
                  <View style={k.idleTapHint}>
                    <Ionicons name="hand-left-outline" size={22} color="rgba(255,255,255,0.8)" />
      ...
      {step >= 1 && step <= 3 && (
        <SafeAreaView style={k.safe} edges={['top', 'bottom']}>
          ...
          <View style={k.header}>
            <Pressable
              style={k.headerBack}
              onPress={() => step === 1 ? resetToIdle() : goTo((step - 1) as Step)}
              hitSlop={10}
            >
          ...
          {step === 1 && (
            ...
            >
              ...
              {!catalogBusy && catalog.length > 0 && (
                <>
                  {/* Packages first */}
                  {catalog.filter(c => c.itemType === 'package').length > 0 && (
                    <>
                  ...
                  {/* Services */}
                  {catalog.filter(c => c.itemType === 'service').length > 0 && (
                    <>
                    ...
                    <View style={k.selectedSummary}>
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={k.selectedPrice}>
                          ${selected.price.toFixed(2)}  ·  {selected.durationMinutes} min
                        </Text>
                      </View>
                      <Pressable
                        style={k.continueBtn}
          ...
          {step === 2 && (
            ...
            >
              ...
              >
                ...
                {/* Phone field */}
                <Text style={k.inputLabel}>Phone Number  (optional — for updates)</Text>
                <TextInput
                  style={k.input}
            ...
            <View style={k.paymentScreen}>
              ...
              <View style={k.paymentRow}>
                ...
                <Pressable
                  style={[
                    k.payCard,
                    payMethod === 'cash' && k.payCardSelected,
                  ]}
                  onPress={() => setPayMethod('cash')}
                >
                  <View style={[k.payIconWrap, payMethod === 'cash' && k.payIconWrapSelected]}>
                    <Ionicons
                      name="cash-outline"
                      size={36}
                  ...
                  <Text style={[k.payCardTitle, payMethod === 'cash' && k.payCardTitleSelected]}>
                    Pay Now
                  </Text>
                ...
                >
                  <View style={[k.payIconWrap, payMethod === 'counter' && k.payIconWrapSelected]}>
                    <Ionicons
                  ...
                  <Text style={[k.payCardTitle, payMethod === 'counter' && k.payCardTitleSelected]}>
                    Pay After
                  </Text>
                  <Text style={k.payCardSub}>
      ...
      {step === 4 && result && (
        ...
        >
          <SafeAreaView style={k.confirmInner}>
            ...
            <Text style={k.confirmName}>Welcome, {result.customerName.split(' ')[0]}!</Text>
...
const k = StyleSheet.create({
  ...
  idleTitle:   { fontSize: 44, fontWeight: '900', color: '#fff', marginBottom: 10 },
  idleSub:     { fontSize: 20, color: 'rgba(255,255,255,0.75)', textAlign: 'center', marginBottom: 60 },
  idleTapHint: { flexDirection: 'row', alignItems: 'center', gap: 12, marginTop: 40 },
  idleTapText: { fontSize: 20, fontWeight: '800', color: 'rgba(255,255,255,0.9)', letterSpacing: 2 },
  ...
  headerX:      { width: 44, height: 44, alignItems: 'center', justifyContent: 'center' },
  ...
  gridSection:  { fontSize: 13, fontWeight: '700', color: '#888', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12, marginTop: 8 },
  ...
  bundleBadge:        {
    alignSelf: 'flex-start', backgroundColor: '#6a0dad', borderRadius: 6,
    paddingHorizontal: 8, paddingVertical: 3, marginBottom: 8,
  },
  bundleText:         { fontSize: 10, fontWeight: '800', color: '#fff', letterSpacing: 0.8 },
  tileName:           { fontSize: 17, fontWeight: '800', color: '#1f1f1f', marginBottom: 4 },
  tileNameSelected:   { color: '#6a0dad' },
  tileDesc:           { fontSize: 13, color: '#888', marginBottom: 10, lineHeight: 18 },
  tilePriceRow:       { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto' as any },
  tilePrice:          { fontSize: 20, fontWeight: '900', color: '#1f1f1f' },
  tileDurChip:        { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#f5f5f5', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 4 },
  tileDur:            { fontSize: 11, color: '#888', fontWeight: '600' },
  tileCheck:          { position: 'absolute', top: 12, right: 12, width: 22, height: 22, borderRadius: 11, backgroundColor: '#6a0dad', alignItems: 'center', justifyContent: 'center' },
  selectedSummary:    {
    position: 'absolute', bottom: 0, left: 0, right: 0,
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: '#fff', padding: 16,
    borderTopWidth: 1, borderTopColor: '#f0f0f0', ...SHADOW,
  },
  ...
  continueBtn: {
    ...
    backgroundColor: '#6a0dad', borderRadius: 14,
  ...
  serviceRecap:{
    flexDirection: 'row', alignItems: 'center', gap: 8,
    backgroundColor: '#f3eafd', borderRadius: 12,
    padding: 12, marginBottom: 24,
  },
  serviceRecapText: { fontSize: 14, fontWeight: '600', color: '#6a0dad', flex: 1 },
  inputLabel: { fontSize: 14, fontWeight: '700', color: '#555', marginBottom: 8, marginTop: 20 },
  input: {
  ...
  paymentTotal:  { fontSize: 24, fontWeight: '900', color: '#1f1f1f', textAlign: 'center' },
  ...
  payIconWrap:      { width: 64, height: 64, borderRadius: 32, backgroundColor: '#f3eafd', alignItems: 'center', justifyContent: 'center' },
  ...
  payCardSub:       { fontSize: 14, color: '#888', textAlign: 'center', lineHeight: 20 },
  payCheck:         { position: 'absolute', top: 12, right: 12, width: 24, height: 24, borderRadius: 12, backgroundColor: '#6a0dad', alignItems: 'center', justifyContent: 'center' },
  confirmBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10,
    backgroundColor: '#6a0dad', borderRadius: 16, paddingVertical: 18, ...SHADOW,
  },
  confirmBtnText: { color: '#fff', fontSize: 18, fontWeight: '800' },

  // ── Step 4: Confirmation ──
  confirmScreen:   { flex: 1 },
  confirmInner:    { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 32, gap: 12 },
  confirmCheck:    { width: 90, height: 90, borderRadius: 45, backgroundColor: 'rgba(255,255,255,0.25)', alignItems: 'center', justifyContent: 'center', marginBottom: 8 },
  confirmTitle:    { fontSize: 36, fontWeight: '900', color: '#fff' },
  confirmName:     { fontSize: 20, color: 'rgba(255,255,255,0.85)', marginBottom: 8 },
  queueNumWrap:    { alignItems: 'center', marginVertical: 8 },
  queueNumLabel:   { fontSize: 14, fontWeight: '600', color: 'rgba(255,255,255,0.7)', textTransform: 'uppercase', letterSpacing: 1 },
  queueNum:        { fontSize: 80, fontWeight: '900', color: '#fff', lineHeight: 90 },
  statsRow:        { flexDirection: 'row', backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 20, padding: 20, gap: 0, width: '100%', maxWidth: 400 },
  ...
  serviceConfirmText: { fontSize: 16, color: 'rgba(255,255,255,0.8)', marginTop: 8 },
  countdown:       { fontSize: 13, color: 'rgba(255,255,255,0.6)', marginTop: 16 },
  countdownTrack:  { width: 200, height: 4, backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 2, overflow: 'hidden' },
  countdownFill:   { height: '100%', backgroundColor: 'rgba(255,255,255,0.6)', borderRadius: 2 },
  doneBtn:         { backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 14, paddingHorizontal: 48, paddingVertical: 16, marginTop: 12 },
